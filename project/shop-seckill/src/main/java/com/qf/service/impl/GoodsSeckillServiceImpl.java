...
public class GoodsSeckillServiceImpl  implements IGoodsSeckillService {
    ...
    public void init() {
        ...
        // key是秒杀的商品，value商品的库存，超时时间可以取秒杀的时长，   设置过期时间为10分钟
...
//        checkUserLogin();
    ...
    //    Goods goods = checkGoodsGstock(id);
        ...
        // 2.扣减库存
       updateGoodsGsotckRedis(goods);
   //     updateGoodsGstock(goods);
    ...
    private void updateGoodsGsotckRedis(Goods goods) {
...
//        orderMapper.insert(order);
    ...
    private void updateGoodsGstock(Goods goods) {
        goodsSeckillMapper.updateGoodsGstock(goods);
    ...
    private Goods checkGoodsGstock(Integer id) {
        ...
        good.setId(id);
        Goods goods = goodsSeckillMapper.selectOne(good);
        if (goods.getGstock() <= 0) {
            throw new SeckillException("商品太火爆了，已经被抢完了，下次再次。。。。");
        }
        return new Goods(id, goods.getGstock().intValue());
    }
